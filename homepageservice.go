...
func (h *HomepageService) getSongUrlInternal(hash string, songName, artist, quality string) SongUrlResponse {
	...
	// 如果获取到播放地址，先同步缓存，再返回本地URL给播放器。
	if len(remoteUrls) > 0 {
		log.Printf("✅ 获取到 %d 个播放地址\n", len(remoteUrls))

		if h.cacheService != nil {
			log.Printf("🎵 开始同步缓存音频文件: %s\n", hash)
			cacheResponse := h.cacheService.CacheAudioFile(hash, remoteUrls, songName, artist, quality)
			if cacheResponse.Success {
				log.Printf("✅ 音频文件缓存成功，将使用本地URL播放: %s -> %s\n", hash, cacheResponse.Data)
				return SongUrlResponse{
					Success:   true,
					Message:   "获取播放地址并缓存成功",
					ErrorCode: 0,
					Data: SongUrlData{
						URLs:   []string{cacheResponse.Data},
						Lyrics: lyricsContent,
					},
				}
			}
			log.Printf("⚠️ 音频文件缓存失败，回退到远程代理播放: %s\n", cacheResponse.Message)
		}

		proxyUrls := make([]string, 0, len(remoteUrls))
		for _, remoteURL := range remoteUrls {
			if h.cacheService != nil {
				proxyUrls = append(proxyUrls, h.cacheService.GetProxyURL(hash, remoteURL))
			} else {
				proxyUrls = append(proxyUrls, remoteURL)
			}
		}
	...
	// 如果没有获取到播放地址，返回失败
	{
		...
		// 保留接口状态，便于前端区分 VIP 权限和普通请求错误。
		if status, ok := apiResponse["status"].(float64); ok {
			statusInt = int(status)
		}
		if errorCode, ok := apiResponse["error_code"].(float64); ok {
			errorCodeInt = int(errorCode)
		...
		if statusInt == 3 {
			message = "该歌曲需要有效的酷狗 VIP，且本地没有可用缓存"
		} else if msg, ok := apiResponse["message"].(string); ok && msg != "" {
			message = msg
		} else if statusInt != 0 {
			message = fmt.Sprintf("获取播放地址失败，状态码: %d", statusInt)
		}
