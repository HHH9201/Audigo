...
func (e *Chromium) Focus() {
	// 控制器初始化尚未完成，或正在窗口消息处理期间（WndProc）被触发时，
	// controller 可能为 nil 或 MoveFocus 是瞬时失败。这些都不应终止进程。
	// 因此这里改为尽力而为：失败仅记录日志，不再调用 errorCallback(os.Exit)。
	if e.controller == nil {
		return
	}
	if err := e.controller.MoveFocus(COREWEBVIEW2_MOVE_FOCUS_REASON_PROGRAMMATIC); err != nil {
		// MoveFocus 偶尔会在窗口尚未完全就绪时返回 0x80070057(参数错误)，
		// 属于无害的瞬时错误，忽略即可。
		e.globalErrorCallback(fmt.Errorf("webview2 MoveFocus failed (ignored): %w", err))
	}
}
