...
// 获取歌曲播放地址和歌词
// songInfo: 歌曲信息对象（包含 hash/songname/author_name），用于生成可读的缓存文件名
const songUrlRequests = new Map();

async function getSongPlayUrls(hash, songInfo) {
    const quality = window.appSettings?.quality?.streamingQuality || 'high';
    const requestKey = `${hash}|${quality}`;
    const pendingRequest = songUrlRequests.get(requestKey);
    if (pendingRequest) {
        console.log('🎵 复用进行中的播放地址请求:', requestKey);
        return pendingRequest;
    }

    const request = getSongPlayUrlsInternal(hash, songInfo, quality);
    songUrlRequests.set(requestKey, request);
    try {
        return await request;
    } finally {
        songUrlRequests.delete(requestKey);
    }
}

async function getSongPlayUrlsInternal(hash, songInfo, quality) {
    try {
        console.log('🎵 正在获取播放地址和歌词...', hash);

        // 提取歌曲名和歌手（用于缓存文件名）
        const songName = (songInfo && (songInfo.songname || songInfo.songName || songInfo.song_name || songInfo.title || songInfo.audio_name || songInfo.name || songInfo.filename)) || '';
        const artistValue = songInfo && (songInfo.author_name || songInfo.authorName || songInfo.artist || songInfo.singer_name || songInfo.singer);
        const artist = Array.isArray(artistValue) ? artistValue.join(', ') : (artistValue || '');
        console.log('🎵 缓存文件名信息:', { hash, songName, artist, quality });
        ...
        // 在线音乐：使用原有逻辑
        window.lastSongUrlError = null;
        const response = await HomepageService.GetSongUrl(hash, songName, artist, quality);
        ...
        if (response.success) {
            ...
            const message = response.message || '无法获取播放地址';
            window.lastSongUrlError = {
                message,
                status: response.status,
                errorCode: response.errorCode
            };
            console.error('❌ 获取播放地址失败:', message);
